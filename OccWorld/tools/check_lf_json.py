...
# Add this to import TransVQVAE and trigger registration
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))
from model.TransVQVAE import TransVQVAE  # This should register TransVQVAE
...
def main():
    ...
    for k in sel:
        it = items[k]
        # 兼容 'image_path' 和 'images'
        img_path = it.get("image_path", "unknown.jpg") if 'image_path' in it else it["images"][0] if isinstance(it.get("images", []), list) else it.get("images", "unknown.jpg")

        base = os.path.basename(img_path).replace(".jpg", "")

        # === 1) RLE → 50×50 token（严格按 F+transpose 还原） ===
        if 'rle_pairs' in it:
            # 兼容 rle_pairs 格式：展开为 50x50 网格
            tk_flat = []
            for id_val, count in it['rle_pairs']:
                tk_flat.extend([id_val] * count)
            if len(tk_flat) != 50*50:
                raise ValueError(f"RLE pairs do not sum to 50x50 grid: sum = {len(tk_flat)}")
            tk50 = np.array(tk_flat).reshape(50, 50)
        else:
            # 原逻辑：使用 parse_tokens_from_item (需要 conversations)
...
#     for k in sel:
#         it = items[k]
#         #img_path = it["images"][0] if isinstance(it["images"], list) else it["images"] 改动，可恢复
#         img_path = it.get("images", [it.get("image_path", "unknown.jpg")])[0] if isinstance(it.get("images", [it.get("image_path", "unknown.jpg")]), list) else it.get("image_path", "unknown.jpg")
#         base = os.path.basename(img_path).replace(".jpg", "")
